...
//    void updateAliveSpec();             // updating the archipelago-wide vector of extant species
    ...
    std::vector<double> calculateAllRates(const std::vector<double> &,
                    const int &n_mainlandSpecies, const int &n_islands);    // calculate per-island rates and global rates
    ...
    std::vector<int> sampleNextEvent(const std::vector<double> &,
                                    std::mt19937_64, const int &);   // draw next event; output -> {event(0-7), species(ID)(,island(0-i))}
    // if global event -> vector.size() = 2, if local -> size = 3

    void speciateGlobalClado(const int&, std::mt19937_64,
                    double time, SpeciesID& maxSpeciesID);         // island species cladogenetically speciates over all islands
    ...
    void goGlobalExtinct(const int&, double time);             // island species goes extinct on all islands it occures on

    void doNextEvent(const std::vector<int>&, const double&,
            std::mt19937_64, double, SpeciesID& maxSpeciesID); // switch-statement that calls event functions
    ...
    std::vector<Species> makeArchiTo1Island() const;   // aggregate all islands in ArchiPylo vector as it would be one and return it
    const std::vector<Island> & returnArchi() const {return mArchipel;}      // return OneIslaArchiPylo vector (and output to file ?)
    // ### CAUTION ###: only const& if assigning this output to a variable copies it
    void printArchi();

private:
    std::vector<double> mGlobalRates;           // vector of rates for global events (3 rates: lambC, lambA, mu)
