...
main_dir <- ""
parameter_file <- "buildup_of_model_pars_global.txt"
dir_name_sims <- "sims/"
subfolder_name <- "mig_divided_by_nIsl-1/"
...
for(y_lim in y_lims) {
  for(sim_name in sim_names) {
    ...
    for(rep in 1 : replicates) {
      ...
      stt_all <- as.matrix(read_csv(paste(main_dir, dir_name_sims,subfolder_name, sim_name, "/rep_", as.character(rep), "_STT.txt", sep = "")))
      island_replicates[[rep]][[1]] = list(stt_all = stt_all)
    }
    island_replicates
    length(island_replicates)
    png(filename = paste(main_dir,"figures/ylim",y_lim,"/",subfolder_name,sim_name,".png",sep=""), type = "cairo", units = "in",
